...
/* harmony import */ var _modules_filters__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./modules/filters */ "./src/js/modules/filters.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_filters__WEBPACK_IMPORTED_MODULE_7__["default"])();
  ...
  Object(_modules_burger__WEBPACK_IMPORTED_MODULE_10__["default"])('.burger-menu', '.burger'); // scrolling('.pageup')
...
var forms = function forms() {
  ...
  }; //! Функция смены имени файла загрузки 
  ...
  form.forEach(function (item) {
    ...
    item.addEventListener('submit', function (e) {
      ...
      statusMessage.classList.add('status'); //! Помещаем новосозданный блок в родителя формы которую перебираем
      ...
      }, 400); //! Создаем изображение и указываем к нему путь 

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp'); //! Вставляем изображение статуса в родителя формы 

      statusMessage.appendChild(statusImg); //!Текстовое оповещение о статусе отправки формы

      var textMessage = document.createElement('div');
      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage); //! Поместить все собранные данные в обьект 

      var formData = new FormData(item);
      var api; //! Находим ближайший родительский блок 

      item.closest('.popup-design') || item.classList.contains('calc-form') ? api = path.designer : api = path.question;
      console.log(api); //! Адресс отправка (в нашем случае локальный файл и данные)

      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData) //! then and catch почитать! Промисы положительно ли выполнен запрос или отрицательно 
      .then(function (res) {
        console.log(res);
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    var matrix = '+7 (___) ___ __ __',
        i = 0,
        //! Регулярные выражения, заменят все НЕ цифры на пустую строку
    def = matrix.replace(/\D/g, ''),
        val = this.value.replace(/\D/g, ''); //! Если пользователь пытается удалить +7 то val.length будет меньше чем  def.length и это не произойдет
...
var modals = function modals() {
  //! Создаем переменную в которую запишем "была ли нажата какая то кнопка"
  var btnPressed = false; //! Назначаем необходимые аргументы для вызова функции 
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        if (e.target) {
          e.preventDefault();
        } //! Таким образом мы узнали был ли клик хоть на один тригер


        btnPressed = true; //! Если передается дейстрой = тру , то удаляем объект
  ...
  function showModalByTyme(selector, time) {
    setTimeout(function () {
      var display; //! Находим все модальные окна

      document.querySelectorAll('[data-modal]').forEach(function (item) {
        //! Находим все скомпелированные браузером стили, и если в перебираемой модалке есть указанный стиль то выполняем нужное действие
        if (getComputedStyle(item).display !== 'none') {
          display = 'block';
        }
      }); //! Если ни одна модалка не показана, то показываем нужное по селектору
  ...
  } //! Функция высчитывающая расстояние скрола 


  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      //! Если не кликнул ни на один триггер и долистал до конца страницы
      //! pageYOffset - сколько уже отлистали от верха страницы | client height - контент видимый на странице | scrollHeight - весь контент страницы (от начала до конца)
      if (!btnPressed && window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
        //! Если условия выполнены то вызываем вручную клик по элементу
  ...
  openByScroll('.fixed-gift'); // showModalByTyme('.popup-consultation', 6000)
};
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  //! Получаем элемнты страницы
  var btn = document.querySelector(trigger); // //! Перебираем массив карточек
  // cards.forEach(card => {
  //     card.classList.add('animated', 'fadeInUp')
  // });
  // //! Вешаем событие на кнопку для показа 
  ...
  //     btn.remove();
  // })
  //! Вешаем событие на кнопку, функция getResourse возвращает промис, поэтому используем .then по выполнению создаем новую карточку и удаляем кнопку

  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResourse"])('assets/db.json').then(function (res) {
      return createCard(res.styles);
    }).catch(function (error) {
      return console.log(error);
    });
    this.remove();
  }); //! Создаем карточку где response полученный элемент массива данных и базы, и для каждого полученного элмента создаем разметку и апендим в родителя wrapper
